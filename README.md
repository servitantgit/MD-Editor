...
js/
  ...
  editor.js                обгортка над EasyMDE: створення, preview-рендер,
                           коректний CodeMirror.refresh(), live-превью зображень
                           прямо в коді (inline image rendering)
...
- **Live перегляд зображень прямо в редакторі** — картинки з markdown
  (`![alt](path)` або `<img>`) візуально рендеряться просто в коді CodeMirror
  замість тексту, тож автор одразу бачив фактичне зображення, не перемикаючись
  в прев'ю. Оригінальний `.md`-джерело в файлі при цьому не змінюється.
